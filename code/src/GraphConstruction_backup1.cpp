...
// New Wiring method
	...
	*/
	...
	int generate_1k_graph_count=0;
		...
		while (out_it != out_end) {
			...
			while (in_it != in_end) {
				...
				bool success = add_edge(vertex(out_it->first, *this->g), vertex(in_it->first, *this->g), *this->g).second;
				if (success) {
					out_it->second--;
					in_it->second--;
			...
			if (in_it == in_end) {
				//std::cout << "Jerry error: couldn't find another vertex to insert: out: " << out_it->first << std::endl;
				out_it++;
			}
	...
	int rewireEdge = 0;
	//vector<int> randomNumExit;
	int randomVertex = utils::generateRandomNumberFrom0TOmax(num_vertices);
	...
	//while (Synthetic_TRIANGLE < 0) {     //choose this line to represent the number of triangles not reconstructed
		...
		/*
		//Exclude vertices that cannot form triangles
		while (1) {
			randomVertex = utils::generateRandomNumberFrom0TOmax(num_vertices);
			for (i = 0; i < randomNumExit.size(); i++)
				if (randomNumExit[i] == randomVertex)
					break;
			if(i==randomNumExit.size())
			break;
		}
		*/
		int target_neighbor, source_neighbor;
		target_neighbor = randomVertex;

		//The probability P determines whether the node is adopted or not
		//std::cout << "edge num=" << ecount << std::endl;
		//float random = rand() % (N + 1) / (float)(N + 1);
		//float p = (float)((float)(FRVN.countVertexOutDegree(randomVertex, *this->g)) / (float)ecount);
		...
		//if (random < p) 
		{
			...
			int fourthVertex;
			/*TODO:This can be optimized to undo the reconnection
			if the number of triangles after reconnecting is smaller
			than the previous figure
			*/
			//if the vertex's outdegree is greater than 2,rewire
			if (out_degree(vertex(target_neighbor, *this->g), *this->g) > 2) 
			{

				//find the random next vertex and delete the edge
				fourthVertex = FRVN.FindRandomVertexNeighbor("out", target_neighbor, *this->g);

				//std::cout << "copy the graph begin." << std::endl;
				this->temp_g->clear();       //clear the graph
				*this->temp_g = *this->g;
				//std::cout << "copy the graph finish." << std::endl;

				//remove two edge
				remove_edge(vertex(target_neighbor, *this->g),
					vertex(fourthVertex, *this->g), *this->g);
				remove_edge(vertex(source_neighbor, *this->g),
					vertex(randomVertex, *this->g), *this->g);

				//add two edge
				bool addSucess_target = add_edge(vertex(target_neighbor, *this->g),
					vertex(randomVertex, *this->g), *this->g).second;
				bool addSucess_source = add_edge(vertex(source_neighbor, *this->g),
					vertex(fourthVertex, *this->g), *this->g).second;
				...
				if (little == true) {
					//if (temp > Synthetic_TRIANGLE && countSubGraph.operator()(*this->g)==1)
					if (temp > Synthetic_TRIANGLE)
					{
					...
					{
						//randomNumExit.push_back(randomVertex);  //if rewire fail,it will not choose this random vertex in next time.
						this->g->clear();       //clear the graph
						*this->g = *this->temp_g;
						//std::cout << "Rewire failed." << std::endl;
					}
				}
				else
				{
