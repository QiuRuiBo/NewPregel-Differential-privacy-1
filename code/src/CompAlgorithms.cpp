...
#pragma omp parallel for 
	for(int ecount = 0; ecount < numedges; ecount++){	
		...
		// float timeelapsed = -1.0;
		// starttime = std::clock();
		
		/* edge_iter e = eit.first;
		for(int cd=0; cd < ecount; cd++)
			e++;
		...
		if(src_replicas.size()>tgt_replicas.size()){
			...
			if(std::find(tgt_replicas.begin(),tgt_replicas.end(),loc_id) == tgt_replicas.end()){
				(*dag->g)[tgt].replica_location.push_back(loc_id);
			}	
		}else{
			...
			if(std::find(src_replicas.begin(),src_replicas.end(),loc_id) == src_replicas.end()){
				(*dag->g)[src].replica_location.push_back(loc_id);
			}
		}
		...
		/* endtime = std::clock();		
		timeelapsed = (float)(endtime - starttime) / (float)CLOCKS_PER_SEC;
		if(timeelapsed > 1e-12){
			printf("edge num: %d, %d-->%d, location id:%d\n",ecount,src,tgt,loc_id);
			printf("src replica:");
			for(int si=0; si<src_replicas.size(); si++){
				printf("%d,\t",src_replicas[si]);
			}
			printf("\n tgt replica:");
			for(int si=0; si<tgt_replicas.size(); si++){
				printf("%d,\t",tgt_replicas[si]);
			}
			printf("\n");
		}
		printf("optimization overhead for one edge assignment is %.4f seconds\n", timeelapsed); */
	}
	...
	#pragma omp parallel for
	for(int vi=0; vi<dag->num_vertices; vi++){
		int master = dag->rnd_generator(0,(*dag->g)[vi].replica_location.size());
		(*dag->g)[vi].master_location = (*dag->g)[vi].replica_location[master];
	}
...
void ContextSwitch(int dc1, int dc2, Optimizer* opt){
	...
	for(int i=0; i<vertexsize; i++){
		tmp_master[i] = (*opt->dag->g)[i].master_location;
	}
...
#pragma omp parallel for
	for(int vi=0; vi<contained_vertices[0].size(); vi++){
		...
		for(int ri =0; ri < (*opt->dag->g)[gv].replica_location.size(); ri++){
			if((*opt->dag->g)[gv].replica_location[ri] == dc1){
				(*opt->dag->g)[gv].replica_location[ri] = dc2;
				break;
			}
		}
		if((*opt->dag->g)[gv].master_location == dc1)
			tmp_master[gv] = dc2;
	}
#pragma omp parallel for
	for(int vi=0; vi<contained_vertices[1].size(); vi++){				
		...
		for(int ri =0; ri < (*opt->dag->g)[gv].replica_location.size(); ri++){
			if((*opt->dag->g)[gv].replica_location[ri] == dc2){
				(*opt->dag->g)[gv].replica_location[ri] = dc1;
				break;
			}
		}
		if((*opt->dag->g)[gv].master_location == dc2)
			tmp_master[gv] = dc1;
	}
#pragma omp parallel for
	for(int vi=0; vi<contained_vertices[0].size(); vi++){
		int gv = contained_vertices[0][vi];
		(*opt->dag->g)[gv].master_location = tmp_master[gv];
		std::sort((*opt->dag->g)[gv].replica_location.begin(),(*opt->dag->g)[gv].replica_location.end());
		std::vector<int>::iterator it = std::unique((*opt->dag->g)[gv].replica_location.begin(),(*opt->dag->g)[gv].replica_location.end());
		(*opt->dag->g)[gv].replica_location.resize(std::distance((*opt->dag->g)[gv].replica_location.begin(),it));
	}
#pragma omp parallel for
	for(int vi=0; vi<contained_vertices[1].size(); vi++){
		int gv = contained_vertices[1][vi];
...
// #pragma omp parallel for
	...
	for(int vi=0; vi<vertexsize; vi++){
		int loc_master = (*opt->dag->g)[vi].master_location;
		if(std::find((*opt->dag->g)[vi].replica_location.begin(),(*opt->dag->g)[vi].replica_location.end(),loc_master)
		== (*opt->dag->g)[vi].replica_location.end())
...
void Optimizer::HPartition(){
	...
	std::vector<int> loads = std::vector<int>(DCs.size(),0);
	for(int vi = 0; vi< dag->num_vertices; vi++){
		loads[(*dag->g)[vi].location_id] ++; //+= num_unassigned_edge[vi];
	}
	omp_set_num_threads(N_THREADS);
	int numoflocks = dag->num_vertices;
	std::vector<omp_lock_t> writelock(numoflocks);
...
#pragma omp parallel for
	for(int ecount = 0; ecount < numedges; ecount++){
		...
		if(intersection.size() > 0){			
			...
			std::vector<int> space = num_unassigned_edge[src] > num_unassigned_edge[tgt] ? (*dag->g)[src].replica_location : (*dag->g)[tgt].replica_location;
			float load = LARGE_FLOAT;
			int least_loaded = -1;
			for(int i=0; i<space.size(); i++){				
				if(loads[space[i]] < load){
			...
			std::vector<int> space = src_replicas.size()>0 ? (*dag->g)[src].replica_location : (*dag->g)[tgt].replica_location;
			float load = LARGE_FLOAT;
			int least_loaded = -1;
			for(int i=0; i<space.size(); i++){				
				if(loads[space[i]] < load){
	...
	*************************************************************************************************/
				...
				if(intersection.size() > 0){
					...
					while(leftovercap > 0 && !candidates.empty()){
						...
						for(int ei=0; ei < connected_edges.size(); ei++){
							...
							orig_master.push_back((*dag->g)[node].master_location);
							...
							orig_replica.push_back((*dag->g)[node].replica_location);
							if(src == v_to_rm ){								
								in_nbr_distribution[tgt][dc] --;
							...
							if(std::find((*dag->g)[node].replica_location.begin(),(*dag->g)[node].replica_location.end(),
								partner_dc) == (*dag->g)[node].replica_location.end()){
								(*dag->g)[node].replica_location.push_back(partner_dc);
							...
							if(in_nbr_distribution[node][dc]==0 && out_nbr_distribution[node][dc]==0){
								...
								if((*dag->g)[node].master_location == dc)
									(*dag->g)[node].master_location = partner_dc;
							}								
							// if(in_nbr_distribution[tgt][partner_dc] == 0 && out_nbr_distribution[tgt][partner_dc]==0){
								// contained_vertices[partner_dc].push_back(tgt);
								// // (*dag->g)[tgt].replica_location.push_back(partner_dc);
							// }
						}
						orig_master.push_back((*dag->g)[v_to_rm].master_location);
						...
						orig_replica.push_back((*dag->g)[v_to_rm].replica_location);
						if((*dag->g)[v_to_rm].master_location == dc)
							(*dag->g)[v_to_rm].master_location = partner_dc;
						...
						for(int di=0; di<(*dag->g)[v_to_rm].replica_location.size(); di++){
							if((*dag->g)[v_to_rm].replica_location[di] == dc){
								(*dag->g)[v_to_rm].replica_location.erase((*dag->g)[v_to_rm].replica_location.begin()+di);
								break;
							}
						}
						...
						if(newCost.first >= currCost.first){
							...
							for(int ei=0; ei<connected_edges.size(); ei++){
								...
								int tgt = target(dag->random_access_edges[connected_edges[ei]], *dag->g);
								int node = (src == v_to_rm? tgt:src);
								(*dag->g)[node].master_location = orig_master[ei];
								...
								(*dag->g)[node].replica_location = orig_replica[ei];
								// if(in_nbr_distribution[node][partner_dc] == 0 && out_nbr_distribution[node][partner_dc] == 0){
									// for(int vi = 0; vi<contained_vertices[partner_dc].size(); vi++){
										// if(contained_vertices[partner_dc][vi] == node){
											// contained_vertices[partner_dc].erase(contained_vertices[partner_dc].begin()+vi);
											// break;
										// }
									// }
								// }
							...
							// if(orig_master == dc)
								// (*dag->g)[v_to_rm].master_location = dc;
							// in_nbr_distribution[v_to_rm][dc] = in_num;
							// out_nbr_distribution[v_to_rm][dc] = out_num;				
							// contained_vertices[dc].push_back(v_to_rm);		
							// // (*dag->g)[v_to_rm].replica_location.push_back(dc);							
							(*dag->g)[v_to_rm].master_location = orig_master[connected_edges.size()];
							...
							(*dag->g)[v_to_rm].replica_location = orig_replica[connected_edges.size()];
	...
	// for(int dc = 0; dc < DCs.size(); dc++){
		// for(int ei=0; ei<edge_distribution[dc].size(); ei++){
			...
			// int src = source(dag->random_access_edges[ecount], *dag->g);
			// int tgt = target(dag->random_access_edges[ecount], *dag->g);	
			// (*dag->g)[src].replica_location.push_back(dc);
			// (*dag->g)[tgt].replica_location.push_back(dc);
		// }
	// }
	for(int v=0; v<dag->num_vertices; v++){
		std::sort((*dag->g)[v].replica_location.begin(),(*dag->g)[v].replica_location.end());
		std::vector<int>::iterator it = std::unique((*dag->g)[v].replica_location.begin(),(*dag->g)[v].replica_location.end());
		(*dag->g)[v].replica_location.resize(std::distance((*dag->g)[v].replica_location.begin(),it));
